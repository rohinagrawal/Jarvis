#!/usr/bin/env python3
"""Validate shared .agents/ assets and thin host adapters.

This script is repo-agnostic: it only inspects the .agents/ folder's own
structure (manifest.json, adapter symlinks, root layout, and any .py tooling
under .agents/ itself). It never inspects or assumes anything about the host
project's language, build system, or dependencies.
"""
...
def check_paths_and_docs(manifest: dict[str, Any], reporter: Reporter) -> None:
    for asset in manifest.get("assets", []):
        ...
        if source_path and source_path.is_file() and source_path.suffix in {".py", ".md", ".json", ".toml"}:
            text = source_path.read_text(encoding="utf-8", errors="replace")
            for marker in ("API_TOKEN=", "SECRET=", "PASSWORD="):
                if marker in text and "<" not in text.split(marker, 1)[1][:20]:
                    reporter.fail(f"{asset_id}: possible committed secret in {source}")
